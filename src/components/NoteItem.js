...
function NoteItem({ id, title, createdAt, body, color }) {
  ...
  return (
    <div className={`note-item note-item__animation ${color}`}>
      ...
      <div className='note-item__action'>
        ...
        <Link aria-label='Add to see detail note' to={`/notes/${id}`} className='note-item__action-btn'><MdOutlineRemoveRedEye /></Link>
      </div>
    </div>
  );
}
